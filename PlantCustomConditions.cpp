...
void hkWizardTurnTargetToSheep(int a1, PlantGroup* a2) {
	if (a2) {
		// LOGI("initial");
		Sub2(0x107CDE8, int, v4, int)(a1);
		Sub2(0x8A8AB0, int, tmp, int, PlantGroup*)(v4, a2);
		// LOGI("passed first 2");
		if (tmp) {
			Sub2(0x76FF5C, int, v5, int)(a1);
			...
			Sub2(0x716354, Effect_FrogAnim*, instance, int, int)(board, effectType);
			...
			Sub(0xC9B7D4, RtWeakPtr<int>*, SexyString*)(&a3, &resourceName);
			int popAnim = (int)a3.get();
			Sub(0xE56260, Effect_PopAnim*, int, int)(instance, popAnim, 0);
			...
			int v19 = fun(a2);
			// LOGI("v19 got");
			Sub(0x735B60, Effect_PopAnim*, SexyVector3*, int)(instance, &coords, v19 + 2);

			//LOGI("set position and layer");

			Sub(0xE56260, Effect_PopAnim*, int)(instance, 1);
			Sub(0x736780, Effect_PopAnim*, int)(instance, 1);
			...
			Sub(0xE54738, std::vector<AnimationSequenceEntry>*, SexyString*, float)(&anims, &startAnim, 0.0);
			Sub(0xE54814, std::vector<AnimationSequenceEntry>*, SexyString*, float, int)(&anims, &loopAnim, MAXFLOAT, 0);
			Sub(0xE54738, std::vector<AnimationSequenceEntry>*, SexyString*, float)(&anims, &jumpUpAnim, 0.0);
			Sub(0xE54738, std::vector<AnimationSequenceEntry>*, SexyString*, float)(&anims, &jumpDownAnim, 0.0);
			// instance->m_timeOfLastStateSwitch = TimeMgr::GetInstance()->m_curTime;

			Sub(0xE56560, Effect_PopAnim*, std::vector<AnimationSequenceEntry>*)(instance, &anims);
			
			// instance + 8 gives ptr
			RtWeakPtr<Effect_PopAnim> sheepPtr;
			sheepPtr.FromOther((RtWeakPtr<Effect_PopAnim>*) &instance->m_thisPtr);
			Sub(0x76E6C0, Zombie*, RtWeakPtr<Effect_PopAnim>*)(zombie, &sheepPtr);

			typedef FrogJumpSubsystem* (*getSubsystem)(Board*, void*);
			FrogJumpSubsystem* subsystem = ((getSubsystem)getActualOffset(0x71E13C))(getBoard(), FrogJumpSubsystem::GetRTClass());
			...
			Sub(0xF4DEE0, PlantGroup*, PlantConditions, float, float)(a2, plant_sheeped, MAXFLOAT, 0.0); // disable plants with condition
...
void hkCanPlantOnGroup(PlantGroup* r0_0, int* gridCell, int plantType, std::vector<int>* reasonsVec) {
	...
	int plantTypeProps = ((getPropsForPlantType)getActualOffset(0x8D2F3C))(plantType, -1, -1, 0);
	...
	for (auto& plantPtr : r0_0->m_plants) {
		...
		int disabled = ((plantIsDisabled)getActualOffset(0xE9AC20))(plant);
		if (disabled) {
			...
			if (!v15) {
				...
				int iceblocked = ((hasIceblockCond)getActualOffset(0xE90964))(plant);
				if (iceblocked) {
...
void initPlantCustomConditionsHooks() {
	PVZ2HookFunction(0xEB8D44, (void*)hkGetConditionsCount, NULL, "GetConditionsCount");
	PVZ2HookFunction(0x7700D8, (void*)hkWizardTurnTargetToSheep, (void**)&oWizardTurnTargetToSheep, "wizard turn target to sheep");
	PVZ2HookFunction(0x76DC4C, (void*)hkRestorePlants, (void**)&oRestorePlants, "wizard restore plants");

	// PVZ2HookFunction(0xEB3EFC, (void*)hkCallback, (void**)&oCallback, "plant group planting callback");
	PVZ2HookFunction(0xF4A9E4, (void*)hkCanPlantOnGroup, (void**)&oCanPlantOnGroup, "can plant on group?"); // Original Fuction has been rewritten, maybe not works
